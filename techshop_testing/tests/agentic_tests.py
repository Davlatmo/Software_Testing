...
#Using absolute path explicitly from the root and looking for the key
env_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '.env'))
print("Looking for .env at:", env_path)
load_dotenv(env_path, override=True)
print("KEY FOUND:", bool(os.getenv("AZURE_API_KEY")))
...
# ---------------------------------------------------------------------------
load_dotenv(r"C:\Users\d.chorshambieva\Downloads\Software_Testing\techshop_testing\.env")
...
# Creating the OpenAI client pointed at our openai endpoint
azure_client = OpenAI(
...
def run_scenario(page, scenario_name: str, instructions: str):
    ...
    for step_num in range(1, MAX_STEPS + 1):
        ...
        try:
            ...
            if action is None:
             steps.append(f"API ERROR: ask_claude returned None")
             return False, steps
...
class TestAgenticHomepage(AgenticTestBase):
    ...
    def test_homepage_loads_correctly(self):
        ...
        self.run_agentic_scenario(
            ...
            instructions=""""
            You are in Techshop homepage.
            1) Assert the page title contains "TechShop"
            2) Assert "Forge Your Setup" is visible
            3) Assert at least 4 product cards are visible
            4)Assert at least one "Add to Cart" button is visible
...
class TestAgenticCart(AgenticTestBase):
    ...
    def test_add_product_to_cart(self):
        ...
        instructions="""
        1) Click "Add to Cart" on the first product card
        2) Wait 1 second
        3) Assert that the page contains "Cart (1)"
        4) The cart count is now 1 and this confirms the item was added -> PASS immediately.
        Do not click anything else after step 3. If Cart (1) is visible -> PASS. Otherwise → FAIL.
        """,
        )
...
class TestAgenticAuth(AgenticTestBase):
    ...
    def test_login_with_demo_account(self):
        ...
        self.run_agentic_scenario(
            name="Login with User Account",
            instructions="""
            1) Enter "dasha@techshop.com" in #email
            2) Enter "dasha123" in #password  
            3) Click the Login button
            4) Wait 2 seconds
            5) Assert "Hi, Dasha" is visible on the page
            6) If "Hi, Dasha" is visible → PASS. Otherwise → FAIL.
            """,  
        ) 
...
class TestAgenticAuthFail(AgenticTestBase):
    ...
    def test_login_fails_with_wrong_password(self):
        ...
        self.run_agentic_scenario(
            ...
            3) Click the Login button
            4) Wait 1 second
            5) Assert an error message that is containing "Invalid credentials" is visible
...
class TestAgenticLogout(AgenticTestBase):
    ...
    def test_logout_after_login(self):
        ...
        self.run_agentic_scenario(
            ...
            instructions="""
            You are on the TechShop login page.
            1) Enter "dasha@techshop.com" in the email field
            2) Enter "dasha123" in the password field
            3) Click the Login button
            4) Wait 2 seconds for redirect to homepage
            5) Assert "Hi, Dasha" is visible in the nav
            6) Click the Logout button
            7) Wait 1 second
            8) Assert "Login" link is visible
            9) Assert "Hi, Dasha" is NOT visible -> PASS
...
class TestAgenticCheckout(AgenticTestBase):
    ...
    def test_full_checkout_flow(self):
     ...
     #Prefilling the form directly so our agent will not strugle with it
     self.page.fill("#firstName", "Davlatmo")
     self.page.fill("#lastName", "Chorshambieva")
     self.page.fill("#address", "Domenico Gentile 26")
     self.page.fill("#city", "Caserta")
     self.page.select_option("#region", "Campania")
     self.page.fill("#zip", "81100")
     self.page.fill("#phone", "70744819")
     self.page.fill("#cardName", "Davlatmo Chorshambieva")
     self.page.fill("#cardNumber", "4111111111111111")
     ...
     self.run_agentic_scenario(
        ...
        instructions="""
        The checkout form is already filled in.
        1) Click #placeOrderBtn
        2) Wait 2 seconds
        3) If "Order Confirmed" is visible -> PASS immediately. Otherwise -> FAIL.
        """,
    )
...
class TestAgenticRegistration(AgenticTestBase):
    ...
    def test_register_new_account(self):
        ...
        unique_email = f"agent_{int(time.time())}@techshop.com"
        self.run_agentic_scenario(
            name="Register New Account",
            instructions=f"""
            You are on the TechShop registration page.
            1) Fill #name with "Yosimin Avazbekova"
            2) Fill #email with "{unique_email}"
            3) Fill #password with "yosi123"
            4) Fill #confirmPassword with "yosi123"
            5) Click the Create Account button
            6) Wait 2 seconds
            7) If the page redirected to homepage and shows "Hi," in the nav -> PASS immediately.
    Otherwise -> FAIL.
            """,
        )
